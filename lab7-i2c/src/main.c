...
// Declaration of "air" variable with structure "Air_parameters_structure"
    ...
    struct Clock_parametrs_structure{
    uint8_t clock_sec;
    ...
    clock.clock_sec=twi_read_nack();
    twi_stop();

    itoa(clock.clock_sec,string,10);
    uart_puts("sec:");
    uart_puts(string);
    uart_putc('\r\n');
...
/*
    ...
    sla++;// Test ACK/NACK value obtained from I2C bus and send info to UART
    */
}